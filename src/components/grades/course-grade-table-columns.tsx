...
type ColumnType = Awaited<ReturnType<typeof api.listCourseGrades>>["listCourseGrades"][number];

export const courseGradeTableColumns: ColumnDef<ColumnType>[] = [
	{
		id: "courseName",
		accessorFn: (c) => c.course.name,
		header: ({column}) => <TableColumnHeader column={column} title="Kurz"/>,
	},
	{
		id: "teacher",
		accessorFn: (c) => c.course.teacher.fullName,
		header: ({column}) => <TableColumnHeader column={column} title="Učitel"/>,
	},
	{
		id: "studentGroup",
	...
	},
	{
		...
		header: ({column}) => <TableColumnHeader column={column} title="Výsledná známka"/>,
	},
	{
		id: "actions",
		header: ({column}) => (
